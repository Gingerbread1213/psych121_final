...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/kay/Desktop/project_psychopy/ospan_with_random_sound/Psych121Project_ospan_nback_lastrun.py',
    ...
    for thisSound_loop in sound_loop:
        ...
        if thisNback_loop != None:
            for paramName in thisNback_loop:
                globals()[paramName] = thisNback_loop[paramName]
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        ...
        for thisNback_loop in nback_loop:
            ...
            # --- Run Routine "nback" ---
            # if trial has changed, end Routine now
            if isinstance(nback_loop, data.TrialHandler2) and thisNback_loop.thisN != nback_loop.thisTrial.thisN:
                continueRoutine = False
            ...
            nback_loop.addData('nback_resp.keys',nback_resp.keys)
            nback_loop.addData('nback_resp.corr', nback_resp.corr)
            if nback_resp.keys != None:  # we had a response
                nback_loop.addData('nback_resp.rt', nback_resp.rt)
                nback_loop.addData('nback_resp.duration', nback_resp.duration)
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        ...
        # completed 1.0 repeats of 'nback_loop'
